...
## После последних обновлений `creat-react-app` не запускается! ##
...
Самое просое, как в последствии оказалось, это просто прописать пути в настройках. В  Windows идем по пути: Правый клик по `"кнопке Пуск"`, в трее слева первая. Дажее  => `"Система"`, => `"Дополнительные параметры системы"` => В открывшемы окне => `"Переменные среды"`. Там уже остается только добавить путь к переменной  `PATH`
...
В некотороых случаях (на windows) помогает обновление npm. О том, как это сделать легко, просто и правильно я рассказывал в своем блоге "Простые советы" в статье [Как обновить npm на  windows.](https://abcinblog.blogspot.com/2020/07/npm-windows.html)
