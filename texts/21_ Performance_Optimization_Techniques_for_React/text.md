...
class PureChildComponent extends React.PureComponent {
    ...
    // Чистые компоненты - это компоненты, которые не перерисовываются, если данные состояния или данные реквизита остаются прежними  
...
// Компонент ниже является оптимизированной версией для компонента по умолчанию

// Компонент не будет повторно визуализироваться, если то же значение реквизита для свойства "name"
...
// Следующая функция принимает объект 'user' в качестве входного параметра в props
...
## 16. Регулирование (Throttling) и отмена (Debouncing) событий.
...
Мы можем использовать сторонние библиотеки для реализации и использования функций регулирования и отладки, одна из таких библиотек - **throttle-debounce** - «регулирование скорости». [По ссылке](https://www.npmjs.com/package/throttle-debounce?source=post_page--------------------------- "NPM throttle-debounce") приведено описание реализации регулирования с использованием этой библиотеки.

## 17. Использование CDN для внешних ресурсов.

Существует несколько сетей доставки контента, предлагаемых Google, Amazon, Microsoft и многими другими компаниями. Эти CDN являются внешними ресурсами, которые можно использовать в вашем приложении. Мы даже можем создавать частные CDN и размещать наши файлы и ресурсы. Полезно использовать CDN по следующим причинам:

### 1. Разные домены:

Браузеры ограничивают одновременное подключение к одному домену определенным номером, оно может варьироваться от браузера к браузеру. Давайте предположим, что количество одновременных разрешенных соединений равно 10. В случае, когда из одного домена можно извлечь 11 ресурсов, только 10 могут быть извлечены одновременно в один момент времени, делая один запрос на ожидание что-либо завершить.

CDN размещены на разных доменах / серверах. Таким образом, файлы ресурсов могут быть распределены между различными доменами, так что несколько запросов могут быть обработаны в один момент времени.

### 2. Файлы могли быть кэшированы.

Несколько веб-сайтов используют эти CDN, поэтому вполне вероятно, что ресурс, к которому вы пытаетесь получить доступ, уже был кэширован в браузере. Поэтому приложение получит доступ к уже кэшированной версии файла и, следовательно, сократит сетевые вызовы и задержку при выполнении скриптов и файлов. Это повышает производительность приложения.

### 3. Высокопроизводительная инфраструктура.

Эти CDN размещаются в огромных компаниях, поэтому доступная инфраструктура огромна, они имеют распределенные центры обработки данных по всему миру. Когда делается запрос к CDN, они обслуживаются через ближайший доступный центр обработки данных, что снижает задержку. Эти компании осуществляют балансировку нагрузки серверов, чтобы гарантировать, что запрос отправляется на ближайший сервер и снижает задержку в сети, улучшая производительность приложения.

Вот некоторые из преимуществ, предоставляемых CDN. Не стесняйтесь использовать CDN или подумайте о создании частных CDN, если безопасность является серьезной проблемой.

## 18. CSS-анимация вместо JavaScript-анимации.

В настоящее время существует несколько способов работы с анимацией. До HTML5 и CSS3 анимация раньше была доменом, который принадлежал JavaScript, но с появлением HTML5 и CSS3 ответственность за предоставление анимации стала накладываться. Анимация может быть обработана даже с помощью CSS3.

Мы можем сформулировать некоторые правила сейчас, поскольку обязанности пересекаются:

>Если что-то может быть достигнуто с помощью CSS поверх JavaScript, сделайте это!
>
>Если что-то может быть достигнуто с помощью HTML поверх JavaScript, сделайте это!

Ниже приведены некоторые причины, чтобы оправдать то же самое:

1. Нарушенные правила и стили CSS не приводят к ошибкам неработающих веб-страниц, в отличие от JavaScript.

2. CSS довольно легок для интерпретации, так как он декларативный, мы можем распараллелить создание представления стиля в памяти и отложить вычисление свойств стиля до тех пор, пока элементы не будут нарисованы.

3. 'Стоимость' загрузки библиотек JavaScript для анимации относительно высока, она потребляет пропускную способность сети и время вычислений.

4. Хотя JavaScript может обеспечить большую оптимизацию, чем CSS, даже оптимизированный код JavaScript может заблокировать пользовательский интерфейс и привести к сбою веб-браузера.

Может быть еще несколько параметров, чтобы решить, какой из них будет более производительным в вашем сценарии. Для более подробной информации, вы можете обратиться к следующему - [CSS Versus JavaScript Animations](https://developers.google.com/web/fundamentals/design-and-ux/animations/css-vs-javascript?source=post_page--------------------------- "CSS Versus JavaScript Animations").

## 19. Включите сжатие Gzip на веб-сервере.

Сжатие - это самый простой способ сохранить пропускную способность сети и ускорить работу приложения. Мы можем включить сетевой ресурс, чтобы сжать их в меньшем размере. Gzip - это алгоритм сжатия данных, способный быстро сжимать и распаковывать файлы. Он может сжимать практически любые типы файлов, такие как изображения, текст, файлы JavaScript, файлы стилей и т. д. На веб-странице Gzip уменьшает объем данных, передаваемых клиенту.

Когда веб-сервер получает запрос, он извлекает данные файла и ищет заголовок «Accept-Encoding», чтобы определить, как кодировать приложение. Если сервер поддерживает сжатие с помощью Gzip, ресурсы сжимаются и отправляются по сети. Сжатые копии каждого ресурса с добавленным заголовком «Content-Encoding», указывающим, что ресурс кодируется с использованием Gzip. Затем браузер распаковывает содержимое в исходную несжатую версию, а затем передает его пользователю.

Несмотря на то, что сжатие Gzip сопряжено с большими затратами, существует задача с интенсивным использованием ЦП, которая запускается при сжатии и распаковке файлов. Даже в этом случае рекомендуется использовать сжатие Gzip для веб-страницы.

Более подробно о [Gzip](https://royal.pingdom.com/can-gzip-compression-really-improve-web-performance/?source=post_page--------------------------- "Can gzip Compression Really Improve Web Performance?").

## 20. Использование Web Workers  для экстенсивных задач CPU.

JavaScript - это однопоточное приложение для обработки всего синхронного выполнения. Во время визуализации веб-страницы требуется выполнение нескольких задач: обработка взаимодействий пользовательского интерфейса, обработка данных ответов, управление элементами DOM, включение анимации и т. Д. Все эти задачи выполняются одним потоком. Web Workers можно рассматривать как вариант, позволяющий снизить нагрузку на выполнение основного потока.

Рабочие потоки (Workers threads) - это фоновые потоки, которые позволяют пользователю выполнять несколько сценариев и выполнять JavaScript, не прерывая основной поток. Всякий раз, когда выполняются длительные задачи, требующие большой загрузки ЦП, эти логические блоки могут выполняться в отдельном потоке с использованием рабочих. Они выполняются в изолированной среде и могут взаимодействовать с основным потоком, используя взаимодействие между процессами. Между тем основной поток может позаботиться о задачах рендеринга и манипулирования DOM.

Более подробно о [Web Workers](https://medium.com/prolanceer/optimizing-react-app-performance-using-web-workers-79266afd4a7?source=post_page--------------------------- "Optimizing React App performance using Web Workers").

## 21. Рендеринг компонента React на стороне сервера.

 Рендеринг на стороне сервера - один из способов уменьшить задержку начальной загрузки страницы. Мы можем разрешить веб-странице загружать начальную страницу с самой стороны сервера, а не выполнять рендеринг на клиенте. Это добавляет значительный импульс в поисковой оптимизации для веб-страницы.

 Рендеринг на стороне сервера - это когда содержимое первого отображаемого компонента отправляется с самого сервера, а не управляется на уровне браузера. Дальнейшие страницы загружаются прямо с клиента. Таким образом, он дает нам возможность начального рендеринга содержимого на стороне сервера и частичной загрузки страницы на стороне клиента для будущих запросов.

 Ниже приведены другие преимущества для этого:

 1. Производительность: исходное содержимое страницы с данными загружается с самого сервера, поэтому нам не нужно помещать загрузчики и спиннеры и ждать загрузки начального компонента после начальной загрузки страницы в браузере.

 2. Оптимизация SEO: сканеры ищут содержимое страницы при начальной загрузке приложения. Когда вы имеете дело с рендерингом на стороне клиента, исходная веб-страница не содержит требуемого компонента, эти компоненты впоследствии отображаются на веб-странице после загрузки скрипта React и других связанных файлов.

 Одним из вариантов рендеринга на стороне сервера является использование сторонних библиотек, таких как «Next.js». Подробности об этом можно найти на следующем [официальном сайте](https://nextjs.org/?source=post_page--------------------------- "nextjs.org").

 Вы также можете найти пример проекта на стороне сервера рендеринга по следующему URL. Чтобы запустить приложение, просто выполните следующие шаги из [репозитория проекта](https://github.com/Mayankgupta688/reactServerRendering?source=post_page--------------------------- "reactServerRendering"):

 ```html
 npm install
 npm start
 ```
Файлы, которые находятся в папке «pages» приложения, являются начальными URL-адресами, которые можно загрузить с помощью рендеринга на стороне сервера.

В случае дальнейшего уточнения по тому же, вы можете связаться со мной для того же.

## Заключение:

Спасибо за ваше время. Пожалуйста, предоставьте комментарии для того же. Если вам понравилась статья, пожалуйста, «аплодируйте» автору Mayank Gupta в [оригинале поста](https://levelup.gitconnected.com/https-medium-com-mayank-gupta-6-88-21-performance-optimizations-techniques-for-react-d15fa52c2349 "21 Performance Optimization Techniques for React. Author: Mayank Gupta. Medium.com")  :-) 

Обратитесь к следующему [репозиторию GitHub](https://github.com/Mayankgupta688/react-demos?source=post_page---------------------------) для примера кода.
