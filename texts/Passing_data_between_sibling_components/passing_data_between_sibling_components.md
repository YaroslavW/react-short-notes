...
***
## Резюме
Весь окончательный код этого приложения вы можете найти на моем [репозитории GitHub](https://github.com/bilousov94/sync_inputs). Как вы видите, передача данных между родственными компонентами React теперь становится простой. Если вы знакомы с Redux, вы можете увидеть много знакомых вещей. Context Api позволяет нам обмениваться данными, которые можно считать «глобальными» для дерева компонентов React. Это решение очень помогает для оптимизации нашего приложения React, но для более сложных проектов с несколькими уровнями вложенности я предпочитаю использовать Redux. Если вы разберетесь со всем этим процессом, как я вам показал, вы легко поймете и Redux.

Я надеюсь, что эта статья вам очень поможет. Мне также интересно узнать о других интересных способах сделать это. Пожалуйста, дайте мне знать, что вы думаете в комментариях ниже.

Автор статьи [Valentyn Bilousov](https://itnext.io/passing-data-between-sibling-components-in-react-using-context-api-and-react-hooks-fce60f12629a) 
